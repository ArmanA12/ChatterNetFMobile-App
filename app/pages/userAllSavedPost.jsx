...
export default function SavedPosts() {
  ...
  const fetchSavedPosts = async () => {
    ...
    try {
      const res = await axios.get(
        'http://chatternet-env.eba-3pqbyabw.ap-south-1.elasticbeanstalk.com/api/v1/post/getAllUserSavedPost',
        {
  ...
  const handleUnsavePost = async (postId) => {
    try {
      const res = await axios.post(
        'http://chatternet-env.eba-3pqbyabw.ap-south-1.elasticbeanstalk.com/api/v1/post/unsavedPost',
        ...
        <View style={styles.postContent}>
          ...
          <View style={styles.buttonContainer}>
            ...
            <TouchableOpacity
              ...
              onPress={() => handleUnsavePost(item.post._id)} // Pass the saved post ID here
            >
  ...
  return (
    <View style={styles.container}>
      <Header headerTitle="Saved Posts" pushRoute="(tabs)/profile" />
      <View style={{ paddingHorizontal: 16, paddingVertical: 10 }}>
        <Text style={styles.sectionTitle}>All Saved Posts ({savedPosts.length})</Text>
      </View>

      {loading ? (
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color="#8383af" />
        </View>
      ) : (
        <FlatList
          data={savedPosts}
